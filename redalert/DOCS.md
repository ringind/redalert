...
Im Web-UI unter **„2 · Steuerung“** stehen nur noch **Start** / **Stop** –
alle Parameter (Effekt, Farbe, Timing, Dauer, `fps`, …) kommen aus der
App-Konfiguration bzw. aus den Overrides der jeweiligen Bridge-Karte (siehe
„1 · Bridges“); die aktuell wirksamen Werte zeigt der Abschnitt „Status“
oben. **Start** / **Stop** starten/stoppen alle eingerichteten Bridges
gleichzeitig; die beiden Knöpfe zeigen per gedrücktem Zustand an, ob der
Effekt gerade läuft.
...
| `police` | **Alarmlicht:** die Lampen einer Bridge werden in zwei Gruppen aufgeteilt (jede zweite Lampe in Kanalreihenfolge); Gruppe 1 blinkt in der Bridge-`color`, Gruppe 2 in `police_color2` (Standard Blau) – die beiden blinken abwechselnd, nie gleichzeitig. `sweep_seconds` ist die Dauer eines vollen Wechsels (beide Gruppen einmal). |
...
| `duel` | **Duell:** zwei Kometen starten an entgegengesetzten Enden der Kanäle – einer in der Bridge-`color`, einer in `police_color2` –, treffen sich in der Mitte und laufen wieder zurück; anders als `meteor` (unabhängig, zufällig) oder `comet` (ein einzelner, deterministischer Umlauf). `sweep_seconds` bestimmt die Periode eines vollen Hin- und Rücklaufs. |
| `sunrise` | **Sonnenaufgang:** ein langsamer Farb- und Helligkeitsbogen für **alle** Lampen einer Bridge gemeinsam (anders als `aurora`s phasenversetzte Palette je Lampe) – wandert zwischen `police_color2` (dunkel/warm) und der Bridge-`color` (hell) und wieder zurück. `sweep_seconds` = Dauer einer Richtung (dunkel → hell); ein voller Zyklus dauert doppelt so lange. |
...
`gc_chase_glitter`, `gc_background_pulse`, `police_color2`,
...
| `effect`        | `pulse` \| `comet` \| `glitter` \| `police` \| `lightning` \| `heartbeat` \| `aurora` \| `rainbow` \| `meteor` \| `wipe` \| `firework` \| `ripple` \| `wave` \| `flicker` \| `strobe` \| `duel` \| `sunrise` \| `chase` \| `neutral` | `pulse`    | Standard-Lichteffekt für Bridges ohne eigene Einstellung, siehe oben. `neutral` sinnvoll nur je Bridge. |
...
| `sweep_seconds` | float (0.3–300.0)  | `1.4`      | Standard für Bridges ohne eigene Einstellung. `comet`: Dauer einer vollen Umrundung. `pulse`/`heartbeat`: Zyklusdauer. `wave`/`strobe`/`duel`: Periodendauer. `sunrise`: Dauer einer Richtung (voller Zyklus = doppelt so lang). |
...
| `police_color2` | Hex-String         | `#0000FF`  | Zweite Farbe – `effect: police` (Farbe der zweiten Lampengruppe, die erste nutzt `color`), `effect: duel` (Farbe des zweiten Kometen) und `effect: sunrise` (dunkles/warmes Ende des Bogens). Je Bridge überschreibbar. |
