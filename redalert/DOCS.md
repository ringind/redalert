...
`meteor_speed`, `firework_interval_ms`, `firework_speed` sind je Bridge
...
| `effect`        | `pulse` \| `comet` \| `glitter` \| `police` \| `lightning` \| `heartbeat` \| `aurora` \| `rainbow` \| `meteor` \| `wipe` \| `firework` \| `chase` \| `neutral` | `pulse`    | Standard-Lichteffekt für Bridges ohne eigene Einstellung, siehe oben. `neutral` sinnvoll nur je Bridge. |
...
| `sweep_seconds` | float (0.3–5.0)    | `1.4`      | Standard für Bridges ohne eigene Einstellung. `comet`: Dauer einer vollen Umrundung. `pulse`: Zyklusdauer. |
...
| `police_color2` | Hex-String         | `#0000FF`  | Nur `effect: police`. Farbe der zweiten Lampengruppe (die erste nutzt `color`). Je Bridge überschreibbar. |
