...
| `effect`        | `pulse` \| `comet` \| `glitter` \| `police` \| `lightning` \| `heartbeat` \| `aurora` \| `rainbow` \| `meteor` \| `wipe` \| `firework` \| `ripple` \| `wave` \| `flicker` \| `strobe` \| `duel` \| `chase` \| `neutral` | `pulse`    | Default light effect for bridges without their own setting, see above. `neutral` only useful per bridge. |
...
| `/health` | GET     | `{status, paired, running, current_preset}` – `paired` is `true` once at least one bridge is paired; `current_preset` is the name of the effect set most recently started via `preset` (`null` on an ad-hoc start without `preset`). Also the container HEALTHCHECK target. |
| `/config` | GET     | Effective configuration incl. `bridges`, `presets` (names of the saved effect sets) and `current_preset` – for the web UI and the Home Assistant integration. |
...
lies a standalone `custom_component` that creates four entities – a
`binary_sensor` (is the effect currently running?), a `switch` (animation
on/off), a `select` (pick & load a saved effect set), and a `sensor` (name
of the currently loaded set). Install via **HACS** (the repo is HACS-capable
...
| `/start` → `already_running` | Call `/stop` first. |
